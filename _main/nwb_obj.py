...
class session_data_nwb():
    ...
    def generate_results_table(self):
        ...
        with h5py.File(self.nwb_path, "r") as h5f:
            ...
            result_by_trial_string = np.array(
                [
                    val.decode("utf-8") if isinstance(val, bytes) else val
                    for val in h5f['intervals']['trials']['HMCF'][:]
                ]
            )
            trial_type = np.array(
                [
                    val.decode("utf-8") if isinstance(val, bytes) else val
                    for val in h5f['intervals']['trials']['trial_type'][:]
                ]
            )
            
            ntrials = len(trial_type)
            ttypearr = np.zeros(ntrials)
            trial_type_as_string = np.char.strip(trial_type.astype(str))
            ttypeGO = np.isin(trial_type_as_string, ["Go", "go", "1"])
            ttypeNoGO = np.isin(trial_type_as_string, ["NoGo", "nogo", "NoGO", "2"])
            ttypearr[ttypeGO] = 1
            ...
            trials_group = h5f['intervals']['trials']
            data = {
    ...
    def get_parameters(self):
        with h5py.File(self.nwb_path, "r") as h5f:
            ...
            values_string = np.array([val for val in h5f['acquisition']['Parameters']['value'][:]])
            keys_strings = np.array([val for val in h5f['acquisition']['Parameters']['key'][:]])
            parameters = {}
